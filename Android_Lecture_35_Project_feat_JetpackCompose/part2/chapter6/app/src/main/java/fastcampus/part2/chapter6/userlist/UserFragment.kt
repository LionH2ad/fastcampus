...
import fastcampus.part2.chapter6.Key.Companion.DB_CHAT_ROOMS
...
import fastcampus.part2.chapter6.databinding.FragmentUserlistBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val userListAdapter = UserAdapter { otherUser ->
            ...
            chatRoomDB.get().addOnSuccessListener {
                ...
                var chatRoomId = ""
                if (it.value != null) { // chatroom 이 존재
                    val chatRoom = it.getValue(ChatRoomItem::class.java)
                    chatRoomId = chatRoom?.chatRoomId ?: ""
                } else { // chatroom 이 없음
                    chatRoomId = UUID.randomUUID().toString() // random으로 생성
        ...
        Firebase.database.reference.child(DB_USERS)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    // forEach 문을 사용하는 것은 나를 제외한 유저의 리스트를 가져오기 위하여
